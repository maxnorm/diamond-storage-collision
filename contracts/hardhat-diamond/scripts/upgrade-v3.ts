...
if (!DIAMOND || !OLD_COUNTER_FACET) {
  console.error("Usage: DIAMOND=<addr> OLD_COUNTER_FACET=<addr> npx hardhat run scripts/upgrade-v3.ts");
  process.exit(1);
}
...
console.log("Upgrading to CounterFacetV3 (incompatible — field reorder)...");
console.log("Diamond:", DIAMOND);
console.log("Old CounterFacet:", OLD_COUNTER_FACET);

const newFacet = await ethers.deployContract("CounterFacetV3", [], { signer });
console.log("New CounterFacetV3:", await newFacet.getAddress());
