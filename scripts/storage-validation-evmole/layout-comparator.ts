...
function compareFields(
  ...
  // If new (planned) has more fields, check that extra fields are mappings or appended safely
  if (newFields.length > oldFields.length) {
    const extraFields = newFields.filter((f) => !oldByOffset.has(f.slotOffset));

    // If all extra fields are mappings, they are undetectable from bytecode — treat as compatible
    if (extraFields.every((f) => f.type.startsWith("mapping("))) {
      return {
        compatible: true,
        message: `Mapping fields undetectable from bytecode: ${oldLabel} has ${oldFields.length} fields, ${newLabel} has ${newFields.length} fields (${extraFields.length} mapping field(s) in planned source)`,
      };
    }

    // New non-mapping fields appended — this is append-safe
    const lastOldOffset =
